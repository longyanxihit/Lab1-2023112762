...
public:
    Graph() : m_randomWalkStart(-1), m_randomWalkFixedLength(false), m_randomWalkLength(0) {}

    static std::string toLower(const std::string& s) {
        std::string res;
        res.reserve(s.size());
        for (char c : s) res.push_back(std::tolower(static_cast<unsigned char>(c)));
        return res;
    }
    ...
    void buildFromWordList(const std::vector<std::string>& words) {
        for (size_t i = 0; i + 1 < words.size(); ++i) addEdge(words[i], words[i + 1]);
    }

    bool loadFromDot(const std::string& dotFile) {
        std::ifstream file(dotFile);
        if (!file.is_open()) return false;
        nodes.clear(); wordToIndex.clear(); adjList.clear();
        std::string line;
        while (std::getline(file, line)) {
            ...
            if (line[posLabel] == '"') {
                ...
                if (posLabelEnd != std::string::npos) {
                    std::string wstr = line.substr(posLabel+1, posLabelEnd-posLabel-1);
    ...
    bool hasWord(const std::string& word) const { return wordToIndex.find(toLower(word)) != wordToIndex.end(); }
    ...
    void setRandomWalkFixedLength(int len) { m_randomWalkFixedLength = true; m_randomWalkLength = len; }
    ...
    void showDirectedGraph(const std::string& inputFilePath) const {
        ...
        for (size_t i = 0; i < nodes.size(); ++i) {
            ...
            if (adjList[i].empty()) std::cout << "无";
            else for (const auto& [v, w] : adjList[i]) std::cout << "\"" << nodes[v] << "\"(" << w << ") ";
            std::cout << "\n";
        }
        std::cout << "========================\n";

        std::string ext = inputFilePath.substr(inputFilePath.find_last_of('.') + 1);
        ...
        if (!isDot) {
            ...
            if (!dotFile) { std::cerr << "无法创建dot文件: " << dotPath << std::endl; return; }
            dotFile << "digraph G {\n  node [shape=box];\n";
            for (size_t u = 0; u < nodes.size(); ++u)
                for (const auto& [v, w] : adjList[u])
                    dotFile << "  \"" << nodes[u] << "\" -> \"" << nodes[v] << "\" [label=\"" << w << "\"];\n";
            dotFile << "}\n";
            dotFile.close();
        }
        ...
        if (ret == 0) std::cout << "有向图已保存至: " << picPath << std::endl;
        else std::cerr << "调用dot命令失败，请确保Graphviz已安装且dot在PATH中。\n";
    }
    ...
    std::string queryBridgeWords(const std::string& word1, const std::string& word2) const {
        ...
        for (size_t k = 0; k < nodes.size(); ++k) {
            ...
            if (it1 != adjList[u].end() && it2 != adjList[k].end()) bridges.push_back(nodes[k]);
        }
        if (bridges.empty()) return "No bridge words from \"" + w1 + "\" to \"" + w2 + "\"!";

        std::ostringstream oss;
        oss << "The bridge word" << (bridges.size() > 1 ? "s" : "") << " from \"" << w1 << "\" to \"" << w2 << "\" "
    ...
    std::string generateNewText(const std::string& inputText) const {
        ...
        for (char ch : inputText) {
            if (std::isalpha(static_cast<unsigned char>(ch))) current.push_back(std::tolower(static_cast<unsigned char>(ch)));
        ...
        for (size_t i = 0; i+1 < words.size(); ++i) {
            ...
            if (u != -1 && v != -1) {
                for (size_t k = 0; k < nodes.size(); ++k) {
                    if (adjList[u].count(k) && adjList[k].count(v)) bridges.push_back(nodes[k]);
                }
            }
            if (!bridges.empty()) {
    ...
    std::string calcShortestPath(const std::string& word1, const std::string& word2) const {
        ...
        std::function<void(int)> dfs = [&](int u) {
            if (u == dst) { paths.push_back(currentPath); return; }
            for (const auto& [v, w] : adjList[u]) {
                if (dist[u] + w == dist[v]) {
    ...
    double calPageRank(const std::string& word) const {
        ...
        for (int iter = 0; iter < maxIter; ++iter) {
            ...
            for (int u = 0; u < n; ++u) if (adjList[u].empty()) danglingSum += pr[u];
            double addToAll = d * danglingSum / n;
            for (int u = 0; u < n; ++u) {
                if (adjList[u].empty()) continue;
                double outSum = outDegreeSum(u);
                for (const auto& [v, w] : adjList[u]) newPr[v] += d * pr[u] * (w / outSum);
            }
    ...
    std::string randomWalk() const {
        ...
        } else current = m_randomWalkStart;
        std::vector<std::string> path;
        path.push_back(nodes[current]);
        std::set<std::pair<int,int>> visitedEdges;
        std::cout << "\n随机游走开始";
        if (m_randomWalkFixedLength) std::cout << "，目标词数: " << m_randomWalkLength;
        std::cout << "\n当前路径: " << nodes[current];
        if (!m_randomWalkFixedLength) {
            while (true) {
                if (kbhit()) {
                    char ch = getch();
                    if (ch == 'q' || ch == 'Q') { path.push_back("INTERRUPT"); break; }
                }
                if (adjList[current].empty()) break;
                std::vector<int> targets, weights;
                for (const auto& [v, w] : adjList[current]) { targets.push_back(v); weights.push_back(w); }
            ...
            if (path.size() == static_cast<size_t>(m_randomWalkLength) && !adjList[current].empty()) {
                bool hasUnvisited = false;
                for (const auto& [v, w] : adjList[current]) {
                    if (visitedEdges.count({current, v}) == 0) { hasUnvisited = true; break; }
                }
...
std::vector<std::string> readWordsFromFile(const std::string& filePath) {
    ...
    while (std::getline(file, line)) {
        for (char ch : line) {
            if (std::isalpha(static_cast<unsigned char>(ch))) currentWord.push_back(ch);
            else flushWord();
        }
        flushWord();
    }
...
// 加载文件（增加格式校验）
bool loadFile(const std::string& path, bool isDot) {
    if (!fileExists(path)) {
    ...
    try {
        if (isDot) {
            if (!g_graph.loadFromDot(path)) {
                std::cerr << "加载.dot文件失败（解析错误或格式不符）。\n";
                return false;
            }
            if (g_graph.nodeCount() == 0) {
                std::cerr << "警告：从文件中未解析出任何节点，文件可能不是有效的.dot文件，或文件为空。\n";
            ...
            if (words.empty()) {
                std::cerr << "警告：文件中未读取到任何有效单词，文件可能不是纯文本文件或内容为空。\n";
                std::cout << "是否仍要使用该文件？（y/n）: ";
                char confirm;
                std::cin >> confirm;
                std::cin.ignore();
        ...
        currentFileName = getFileStem(path);
        g_graph.showDirectedGraph(path);
        return true;
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
        bool isDot = (ext == "dot");
        if (!loadFile(path, isDot)) return 1;
    } else {
        // 循环直到成功加载文件或用户选择退出
        bool loaded = false;
        while (!loaded) {
    ...
    while (true) {
        ...
        switch (choice) {
            case 1:
                while (!backToMenu) {
                    std::cout << "\n[桥接词查询] 请输入两个单词（输入 'back' 返回菜单，输入 'exit' 或 'quit' 退出程序）: ";
                    std::string line;
                    std::getline(std::cin, line);
                    if (isExitCommand(line)) return 0;
                    if (isBackCommand(line)) { backToMenu = true; break; }
                    if (line.empty()) continue;

                    std::vector<std::string> words;
                    std::string cur;
                    for (char ch : line) {
                        if (std::isalpha(static_cast<unsigned char>(ch))) cur.push_back(ch);
                    ...
                    if (words.size() == 1) {
                        ...
                        if (isBackCommand(w2)) { backToMenu = true; break; }
            ...
            case 2:
                while (!backToMenu) {
                    std::cout << "\n[生成新文本] 请输入一行文本（输入 'back' 返回菜单，输入 'exit' 或 'quit' 退出程序）: ";
                    std::string text;
                    std::getline(std::cin, text);
                    if (isExitCommand(text)) return 0;
                    if (isBackCommand(text)) { backToMenu = true; break; }
            ...
            case 3:
                while (!backToMenu) {
                    std::cout << "\n[最短路径] 请输入两个单词（输入 'back' 返回菜单，输入 'exit' 或 'quit' 退出程序）: ";
                    std::string line;
                    std::getline(std::cin, line);
                    if (isExitCommand(line)) return 0;
                    if (isBackCommand(line)) { backToMenu = true; break; }
            ...
            case 4:
                while (!backToMenu) {
                    std::cout << "\n[单源最短路径] 请输入一个单词（输入 'back' 返回菜单，输入 'exit' 或 'quit' 退出程序）: ";
                    std::string word;
                    std::getline(std::cin, word);
                    if (isExitCommand(word)) return 0;
                    if (isBackCommand(word)) { backToMenu = true; break; }
            ...
            case 5:
                while (!backToMenu) {
                    std::cout << "\n[PageRank] 请输入一个单词（输入 'back' 返回菜单，输入 'exit' 或 'quit' 退出程序）: ";
                    std::string word;
                    std::getline(std::cin, word);
                    if (isExitCommand(word)) return 0;
                    if (isBackCommand(word)) { backToMenu = true; break; }
            ...
            case 6:
                while (!backToMenu) {
                    std::cout << "\n[随机游走] 输入 'back' 返回菜单，输入 'exit' 或 'quit' 退出程序，其他键开始设置...\n";
                    std::string input;
                    std::getline(std::cin, input);
                    if (isExitCommand(input)) return 0;
                    if (isBackCommand(input)) { backToMenu = true; break; }
                    ...
                    std::cin >> startChoice;
                    std::cin.ignore();
                    if (startChoice == 1) g_graph.setRandomWalkRandomStart();
                    else if (startChoice == 2) {
                        std::cout << "请输入起始词: ";
                        std::string word;
                        std::getline(std::cin, word);
                        std::string w = extractFirstWord(word);
                        if (w.empty() || !g_graph.hasWord(w)) {
                            std::cout << "单词无效或不在图中，将随机选择起始词。\n";
                            g_graph.setRandomWalkRandomStart();
                        } else g_graph.setRandomWalkStart(w);
                    } else {
                    ...
                    if (lenChoice == 1) {
                        ...
                        std::cin >> n;
                        std::cin.ignore();
                        if (n <= 0) { n = 10; std::cout << "词数必须为正数，使用默认值10。\n"; }
                        g_graph.setRandomWalkFixedLength(n);
                    } else if (lenChoice == 2) g_graph.setRandomWalkInfinite();
                    else {
                        std::cout << "无效选择，使用不定词数模式。\n";
                        g_graph.setRandomWalkInfinite();
                    }

                    std::string walk = g_graph.randomWalk();
                    std::cout << "\n随机游走路径: " << walk << std::endl;

                    std::string dir = getParentDir(currentFilePath);
                    std::string stem = getFileStem(currentFilePath);
                    std::string outPath = dir + stem + "random.txt";
                    std::ofstream ofs(outPath);
                    if (ofs) {
                        ofs << walk;
                        std::cout << "随机游走文本已保存至: " << outPath << std::endl;
                    } else std::cerr << "无法保存文件。" << std::endl;
                }
                break;
            case 7: {
                std::cout << "\n=== 打开新文件 ===\n";
